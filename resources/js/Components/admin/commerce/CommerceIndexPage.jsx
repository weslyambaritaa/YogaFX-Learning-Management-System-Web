...
export default function CommerceIndexPage({
    ...
    const [activeSectionKey, setActiveSectionKey] = useState(
        sectionList[0]?.key ?? null,
    );

    useEffect(() => {
        if (!sectionList.some((section) => section.key === activeSectionKey)) {
            setActiveSectionKey(sectionList[0]?.key ?? null);
        }
    }, [activeSectionKey, sectionList]);

    const activeSection =
        sectionList.find((section) => section.key === activeSectionKey) ??
        sectionList[0] ??
        null;

    const baseQuery = sectionList.reduce((carry, section) => {
        ...
        >
            ...
            <div className="py-12">
                <div className="mx-auto max-w-7xl space-y-6 px-4 sm:px-6 lg:px-8">
                    {sectionList.length ? (
                        <div className="rounded-[5px] border border-slate-200 bg-white p-2 shadow-sm">
                                ...
                                {sectionList.map((section) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={[
                                                'inline-flex items-center gap-2 rounded-[5px] px-4 py-2 text-sm font-medium transition',
                                                isActive
