...
class StudentController extends Controller
{
    ...
    public function studentsIndex(Request $request): Response
    {
        ...
        $tierFilter = (string) $request->input(
            'access_tier_id',
            '',
        );
        ...
        $students = User::query()
            ...
            ->paginate(
                ...
                [
                    ...
                    /*
                     * Needed by hasCompletedStudentProfile()
                     * for the can_impersonate flag.
                     */
                    'whatsapp',
                    'country',
                    'birth_date',
                    ...
                    /*
                     * MasterClass-only profile fields.
                     */
                    'tshirt_size',
                    'favorite_song',
    ...
    public function studentsStore(
        AdminStudentStoreRequest $request,
    ): RedirectResponse {
        $validated =
            $request->validated();
    ...
    public function studentsEdit(
        ...
        $emergencyWhatsapp =
            CountryDirectory::splitPhoneNumber(
                $student->emergency_contact_whatsapp,
                $student->country,
            );
    ...
    public function studentsUpdate(
        AdminStudentUpdateRequest $request,
        User $student,
    ): RedirectResponse {
        abort_unless(
            $student->isStudent(),
        ...
        /*
         * These fields are frontend/helper fields.
         * Only the combined WhatsApp values are stored.
         */
        unset(
        ...
        $student->setStudentAccountStatus(
            $nextAccountStatus,
        );
        ...
        return redirect()
            ->route(
                $this->studentDetailRouteNameForContext(
                    $managementContext,
                ),
                $student,
            )
    ...
    public function resetProgress(
        ...
        $this->deleteAssignmentMediaPaths(
            $deletedAssignmentMediaPaths,
        );
    ...
    public function resetProgressScope(
        Request $request,
        User $student,
        string $scope,
    ): RedirectResponse {
    ...
    public function destroy(
        ...
        $assignmentMediaPaths =
            DB::transaction(
                function () use ($student) {
                    ...
                    /*
                     * Abandoned or retried checkout
                     * attempts may create invoices and
                     * pending registrations without a
                     * user_id. Match by email as well.
                     */
        ...
        $certificateFiles->each(
            fn (string $path) =>
                $this->bunnyStorage->delete(
                    $path,
                ),
        );
    ...
    private function studentDetailRouteNameForContext(
    ...
    private function studentIndexRouteNameForContext(
        string $managementContext,
    ): string {
        return $managementContext ===
            'student_progress'
    ...
    private function resetAllLearningProgress(
        User $student,
    ): Collection {
        $this->resetAssessmentProgress(
            $student,
        );
    ...
    private function resetVideoProgress(
        User $student,
    ): Collection {
        LessonProgress::query()
            ->where(
    ...
    private function resetAssessmentProgress(
        User $student,
    ): Collection {
        $attemptIds =
            AssessmentAttempt::query()
    ...
    private function resetLessonProgress(
        User $student,
    ): Collection {
        LessonProgress::query()
            ->where(
    ...
    private function resetModuleProgress(
        User $student,
    ): Collection {
        $this->resetAssessmentProgress(
            $student,
    ...
    private function resetNonLessonModuleProgress(
        User $student,
    ): Collection {
        $assignmentMediaPaths =
        ...
        CertificateDownloadEvent::query()
            ->where(
                'user_id',
                $student->id,
            )
    ...
    private function deleteAssignmentMediaPaths(
        Collection $paths,
    ): void {
        $paths
            ->filter(
